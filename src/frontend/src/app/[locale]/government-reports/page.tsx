...
'50tawi': { th:'ใบรับรองการหักภาษี ณ ที่จ่าย', en:'Withholding Tax Certificate (50 Tawi)' },
 ...
 :'border-hairline hover:border-hairline hover:bg-surface-raised'
 }`}
 >
 <div className="flex items-start gap-3">
 <div
 className={`flex-shrink-0 w-10 h-10 rounded-md flex items-center justify-center font-bold text-xs ${
 isSelected ?'bg-brand text-white' :'bg-surface-raised text-ink-muted'
 }`}
 >
 ...
 {[1, 2, 3, 4].map((i) => (
 <div key={i} className="h-16 bg-surface-raised animate-pulse rounded-md" />
 ))}
 </div>
 ) : reports.length === 0 ? (
 ...
 <tbody>
 {reports.map((report) => (
 <tr key={report.id} className="border-b hover:bg-surface-raised transition-colors">
 ...
 <td className="px-4 py-3 text-ink-soft">{report.period}</td>
 <td className="px-4 py-3">
 <span
 ...
 <div
 key={report.id}
 className="border rounded-md p-4 bg-surface"
 >
 ...
 <p className="text-xs text-ink-muted mt-1">{formatDate(report.generatedAt)}</p>
 </div>
 ))}
 </div>
 </>
