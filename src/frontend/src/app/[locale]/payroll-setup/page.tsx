...
export default function PayrollSetupPage() {
 ...
 <th className="text-left px-4 py-3 font-semibold text-ink-muted">{t('earnings.code')}</th>
 <th className="text-left px-4 py-3 font-semibold text-ink-muted">{t('earnings.name')}</th>
 <th className="text-left px-4 py-3 font-semibold text-ink-muted hidden sm:table-cell">Category</th>
 <th className="text-center px-4 py-3 font-semibold text-ink-muted">{t('earnings.taxable')}</th>
 <th className="text-center px-4 py-3 font-semibold text-ink-muted">{t('status')}</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-100">
 {earningTypes.map((et) => (
 <tr key={et.id} className="hover:bg-surface-raised transition">
 <td className="px-4 py-3">
 <span className="font-mono text-xs bg-surface-raised px-2 py-1 rounded">{et.code}</span>
 </td>
 <td className="px-4 py-3">
 <p className="font-medium text-ink">{et.nameEn}</p>
 <p className="text-xs text-ink-muted">{et.nameTh}</p>
 </td>
 <td className="px-4 py-3 hidden sm:table-cell">
 <span className="text-ink-muted">{categoryLabel[et.category] ?? et.category}</span>
 </td>
 ...
 <thead>
 <tr className="border-b bg-surface-raised">
 <th className="text-left px-4 py-3 font-semibold text-ink-muted">{t('deductions.code')}</th>
 <th className="text-left px-4 py-3 font-semibold text-ink-muted">{t('deductions.name')}</th>
 <th className="text-left px-4 py-3 font-semibold text-ink-muted hidden sm:table-cell">Category</th>
 <th className="text-center px-4 py-3 font-semibold text-ink-muted">{t('deductions.mandatory')}</th>
 <th className="text-center px-4 py-3 font-semibold text-ink-muted">{t('status')}</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-100">
 {deductionTypes.map((dt) => (
 <tr key={dt.id} className="hover:bg-surface-raised transition">
 <td className="px-4 py-3">
 <span className="font-mono text-xs bg-surface-raised px-2 py-1 rounded">{dt.code}</span>
 </td>
 <td className="px-4 py-3">
 <p className="font-medium text-ink">{dt.nameEn}</p>
 <p className="text-xs text-ink-muted">{dt.nameTh}</p>
 </td>
 <td className="px-4 py-3 hidden sm:table-cell">
 <span className="text-ink-muted">{categoryLabel[dt.category] ?? dt.category}</span>
 </td>
 <td className="px-4 py-3 text-center">
 {dt.mandatory ? (
 ...
 <div className="space-y-6">
 <div>
 <h3 className="text-base font-semibold text-ink mb-1">{t('tax.title')}</h3>
 <p className="text-sm text-ink-muted">{t('tax.progressiveRates')}</p>
 </div>
 ...
 <thead>
 <tr className="border-b bg-surface-raised">
 <th className="text-left px-4 py-3 font-semibold text-ink-muted">{t('tax.incomeRange')}</th>
 <th className="text-right px-4 py-3 font-semibold text-ink-muted">{t('tax.rate')}</th>
 <th className="text-left px-4 py-3 font-semibold text-ink-muted hidden md:table-cell">Description</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-100">
 {taxBrackets.map((bracket, idx) => (
 <tr key={idx} className="hover:bg-surface-raised transition">
 ...
 <td className="px-4 py-3 text-ink-muted hidden md:table-cell">{bracket.descriptionEn}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div>
 <p className="text-ink-muted">{t('tax.employeeRate')}</p>
 <p className="font-semibold text-ink mt-1">5%</p>
 </div>
 <div>
 <p className="text-ink-muted">{t('tax.employerRate')}</p>
 <p className="font-semibold text-ink mt-1">5%</p>
 </div>
 <div>
 <p className="text-ink-muted">{t('tax.maxWage')}</p>
 <p className="font-semibold text-ink mt-1">{formatCurrency(15000,'THB')}</p>
 </div>
 <div>
 <p className="text-ink-muted">{t('tax.maxContribution')}</p>
 ...
 <thead>
 <tr className="border-b bg-surface-raised">
 <th className="text-left px-4 py-3 font-semibold text-ink-muted">Code</th>
 <th className="text-left px-4 py-3 font-semibold text-ink-muted">Bank Name</th>
 <th className="text-left px-4 py-3 font-semibold text-ink-muted hidden sm:table-cell">SWIFT</th>
 <th className="text-center px-4 py-3 font-semibold text-ink-muted">{t('status')}</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-100">
 {banks.map((bank) => (
 <tr key={bank.code} className="hover:bg-surface-raised transition">
 <td className="px-4 py-3">
 <span className="font-mono text-xs bg-surface-raised px-2 py-1 rounded">{bank.code}</span>
 </td>
 <td className="px-4 py-3">
 <p className="font-medium text-ink">{bank.nameEn}</p>
 <p className="text-xs text-ink-muted">{bank.nameTh}</p>
 </td>
 <td className="px-4 py-3 hidden sm:table-cell">
 <span className="font-mono text-xs text-ink-muted">{bank.swiftCode}</span>
 </td>
 <td className="px-4 py-3 text-center">
