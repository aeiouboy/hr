      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              {hiPoList.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-8">{t('noHiPo')}</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {hiPoList.map((emp) => (
                        <tr key={emp.id} className="border-b last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30">
        ...
        <Card>
          ...
          <CardContent>
            <div className="mb-4 flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
              <span className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded bg-gray-300 dark:bg-gray-600" /> {t('nineBox.performance')} →</span>
                  ...
                  <div className="mt-1 space-y-1">
                    {cell.employees.slice(0, 3).map((emp) => (
                      <div key={emp.id} className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-gray-600 dark:text-gray-400 truncate">{emp.name}</span>
                      </div>
                    ))}
                    {cell.employees.length > 3 && <p className="text-xs text-gray-400 dark:text-gray-500">+{cell.employees.length - 3} more</p>}
                  </div>
                </div>
              ))}
            </div>
