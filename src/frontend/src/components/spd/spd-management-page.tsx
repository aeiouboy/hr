  ...
  const renderTimeRecords = () => (
    <Card>
      ...
      <CardContent className="p-5 sm:p-6 lg:p-8 space-y-4">
        ...
        <div className="hidden lg:block overflow-x-auto rounded-xl border dark:border-gray-700">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredTimeRecords.map((record) => {
                ...
                return (
                  <tr key={record.id} className="border-b dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30">
            ...
            <CardContent className="p-5 sm:p-6 lg:p-8">
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2">
                {displayed.map((day, index) => (
                  <div key={`${day.date}-${day.day}`} className={cn('rounded-lg border p-3', SHIFT_COLORS[day.shift])}>
                    <p className="text-xs font-semibold">{day.day}</p>
                    <p className="text-xs opacity-75">{day.date}</p>
                    {isEditing ? (
                      <select
                        className="mt-2 w-full rounded border dark:border-gray-600 bg-white dark:bg-gray-800 px-2 py-1 text-xs text-gray-800 dark:text-gray-100"
                        value={day.shift}
                        onChange={(event) =>
