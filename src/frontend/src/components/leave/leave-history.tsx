...
export function LeaveHistory({
  ...
  return (
    <Card>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <select
              ...
              className="px-3 py-1.5 border dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-800 dark:text-gray-200"
            >
          ...
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                    ...
                    <th className="text-left text-xs font-medium text-gray-500 dark:text-gray-400 px-6 py-3">Date Range</th>
                    <th className="text-center text-xs font-medium text-gray-500 dark:text-gray-400 px-6 py-3">{t('days')}</th>
                    <th className="text-center text-xs font-medium text-gray-500 dark:text-gray-400 px-6 py-3">Status</th>
                    <th className="text-left text-xs font-medium text-gray-500 dark:text-gray-400 px-6 py-3">{t('approvedBy')}</th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {paginatedItems.map((req) => {
                    ...
                    return (
                      <tr key={req.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition cursor-pointer" onClick={() => onViewDetail(req)}>
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-2">
                            <div className={cn('w-2 h-2 rounded-full', colors.bar)} />
                            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{req.typeNameEn}</span>
                          </div>
                        </td>
                        <td className="px-6 py-3 text-sm text-gray-600 dark:text-gray-400">
                          {req.startDate === req.endDate ? req.startDate : `${req.startDate} — ${req.endDate}`}
                        </td>
                        <td className="px-6 py-3 text-sm text-center text-gray-900 dark:text-gray-100 font-medium">
                          {req.days}{req.halfDay ? ` (${req.halfDay === 'morning' ? 'AM' : 'PM'})` : ''}
                        </td>
                        <td className="px-6 py-3 text-center">
                          <Badge variant={STATUS_VARIANTS[req.status]}>{t(`status.${req.status}`)}</Badge>
                        </td>
                        <td className="px-6 py-3 text-sm text-gray-600 dark:text-gray-400">{req.approvedByName || '—'}</td>
            ...
            <div className="md:hidden divide-y divide-gray-100 dark:divide-gray-700">
              {paginatedItems.map((req) => {
                const colors = LEAVE_TYPE_COLORS[req.type as LeaveType] || LEAVE_TYPE_COLORS.annual;
                return (
                  <div key={req.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition cursor-pointer" onClick={() => onViewDetail(req)}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{req.typeNameEn}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">{req.startDate} · {req.days} {t('days')}</p>
                        </div>
                      </div>
                      <Badge variant={STATUS_VARIANTS[req.status]}>{t(`status.${req.status}`)}</Badge>
                    </div>
      ...
      >
        <div className="space-y-3">
          <p className="text-sm text-gray-600 dark:text-gray-400">{t('cancellationReasonPrompt')}</p>
          <textarea
            value={cancelReason}
