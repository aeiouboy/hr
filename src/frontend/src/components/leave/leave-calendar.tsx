...
export function LeaveCalendar({ events, holidays, onDateClick }: LeaveCalendarProps) {
 ...
 <button onClick={goToToday} className="text-sm font-medium text-ink min-w-[160px] text-center hover:text-brand transition">
 {monthName}
 </button>
 ...
 {WEEKDAYS.map((day) => (
 <div key={day} className={cn('text-center text-xs font-medium py-2', day ==='Sun' || day ==='Sat' ?'text-red-400' :'text-ink-muted')}>
 {day}
 </div>
 ))}
 </div>
 ...
 <div key={`empty-${i}`} className="min-h-[60px] md:min-h-[80px] border-t border-gray-100 border-hairline" />
 ))}
