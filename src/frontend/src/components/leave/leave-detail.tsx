...
'w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0',
 ...
 <div className="w-px h-full bg-surface-raised min-h-[16px]" />
 </div>
 <div className="pb-4">
 <p className="text-sm font-medium text-ink">{label}</p>
 {date && <p className="text-xs text-ink-muted">{new Date(date).toLocaleDateString('en-US', { dateStyle:'medium' })} {new Date(date).toLocaleTimeString('en-US', { timeStyle:'short' })}</p>}
...
export function LeaveDetail({ request, open, onClose, onCancel }: LeaveDetailProps) {
 ...
 <p className="p-3 bg-surface-raised rounded-md text-sm">{request.reason}</p>
 </div>
 ...
 <p className="text-sm font-medium text-ink mb-3">Status Timeline</p>
 <div>
 <TimelineStep
