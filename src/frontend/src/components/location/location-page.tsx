  ...
  return (
    <div>
      ...
      >
        ...
        <span className="text-xs text-gray-400 dark:text-gray-500">{node.headcount}</span>
      </div>
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            {!selectedLocation ? (
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-12">{t('selectLocation')}</p>
            ) : (
              <div className="space-y-4">
                ...
                {selectedLocation.children.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{t('childLocations')} ({selectedLocation.children.length})</h4>
                    <div className="space-y-2">
                      {selectedLocation.children.map((child) => (
                        <div key={child.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700" onClick={() => setSelectedLocation(child)}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-gray-400 dark:text-gray-500">{child.headcount}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-8 text-center">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">{t('viewOnMap')}</p>
                </div>
              </div>
            )}
          </CardContent>
