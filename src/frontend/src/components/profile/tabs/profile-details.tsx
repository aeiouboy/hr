  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title={t('profileDetails.education')} icon={<GraduationCap className="h-5 w-5" />} collapsible>
        {education.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400 text-center py-4">{t('common.noData')}</p>
        ) : (
          <div className="space-y-3">
            {education.map((edu, i) => (
              <div key={i} className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                ...
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-gray-100">{edu.degree as string}</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400">{edu.institution as string}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">{edu.major as string}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{t('profileDetails.graduationYear')}: {edu.graduationYear as number}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {prevEmployment.map((job, i) => (
              <div key={i} className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                ...
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-gray-100">{job.jobTitle}</h4>
              ...
              <tbody>
                {languages.map((lang, i) => (
                  <tr key={i} className="border-b dark:border-gray-700 last:border-0">
                    <td className="py-2 px-2 font-medium text-gray-900 dark:text-gray-100">{lang.language}</td>
                    <td className="py-2 px-2 text-gray-600 dark:text-gray-400">{lang.reading}</td>
                    <td className="py-2 px-2 text-gray-600 dark:text-gray-400">{lang.writing}</td>
                    <td className="py-2 px-2 text-gray-600 dark:text-gray-400">{lang.speaking}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="space-y-3">
            {certifications.map((cert, i) => (
              <div key={i} className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                <h4 className="font-medium text-gray-900 dark:text-gray-100">{cert.certName}</h4>
          ...
          <div className="space-y-3">
            {awards.map((award, i) => (
              <div key={i} className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                ...
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-gray-100">{award.awardName}</h4>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{formatDate(award.awardDate, 'medium', locale)}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <SectionCard title={t('profileDetails.mobility')} icon={<MapPin className="h-5 w-5" />} collapsible defaultOpen={false}>
          <div className="space-y-2">
            ...
            {(mobility.preferredLocations as string[])?.length > 0 && (
              <div>
                <span className="text-sm text-gray-500 dark:text-gray-400">{t('profileDetails.preferredLocations')}:</span>
                <div className="flex flex-wrap gap-2 mt-1">
                  {(mobility.preferredLocations as string[]).map((loc) => (
                    <span key={loc} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-1 rounded-full">{loc}</span>
                  ))}
                </div>
              </div>
            )}
