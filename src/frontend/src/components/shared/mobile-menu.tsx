...
export function MobileMenu() {
 ...
 <nav
 className="fixed inset-y-0 left-0 w-72 bg-surface shadow-xl z-50 lg:hidden flex flex-col"
 ...
 <button
 onClick={() => setMobileMenuOpen(false)}
 className="p-2 hover:bg-surface-raised rounded-md min-h-[44px] min-w-[44px]"
 ...
 <div className="px-4 py-3 border-b border-hairline bg-surface-raised">
 ...
 <p className="text-xs text-ink-muted">{roles[0]}</p>
 </div>
 </div>
 </div>
 ...
 return (
 <div key={group.title}>
 <h3 className="px-4 pt-4 pb-1 text-xs font-semibold text-ink-muted uppercase tracking-wider">
 {group.title}
 </h3>
 <ul>
 {filtered.map((item) => {
...
'flex items-center gap-3 px-4 py-3 transition text-sm',
 ...
 <div className="border-t border-hairline mt-2 pt-2 pb-4">
 <button
 onClick={() => {
 setMobileMenuOpen(false);
 clearUser();
