...
export function Modal({ open, onClose, title, children, footer, className }: ModalProps) {
 ...
 ref={contentRef}
 className={cn(
'bg-surface w-full overflow-y-auto shadow-xl ',
 // Mobile bottom sheet
'fixed bottom-0 inset-x-0 rounded-t-2xl max-h-[85vh]',
'animate-[slide-up_0.3s_ease-out]',
 // Desktop centered dialog
'md:static md:rounded-md md:max-w-lg md:max-h-none',
'md:animate-none md:mx-auto',
 ...
 {title && (
 <div className="flex items-center justify-between border-b px-6 py-4 border-hairline">
 <h2 className="text-lg font-semibold text-ink">{title}</h2>
 <button
 onClick={onClose}
 className="rounded-md p-1 hover:bg-surface-raised transition-colors hover:bg-surface-raised"
 aria-label="Close"
 >
 ...
 {footer && <div className="border-t px-6 py-4 border-hairline">{footer}</div>}
 </div>
 </div>
 </dialog>
 );
