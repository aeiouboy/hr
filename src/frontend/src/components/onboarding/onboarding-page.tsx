...
const STATUS_ICON: Record<ItemStatus, React.ReactNode> = {
  ...
  pending: <Circle className="h-4 w-4 text-gray-300 dark:text-gray-600" />,
  not_applicable: <Circle className="h-4 w-4 text-gray-200 dark:text-gray-600" />,
};
          ...
          <CardContent>
            <div className="mb-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm text-gray-500 dark:text-gray-400">{t('overallProgress')}</span>
            ...
            {(['pre_boarding', 'day_one', 'orientation', 'probation'] as const).map((phase) => {
              ...
              return (
                <div key={phase} className="mb-6">
                  <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3 uppercase">{t(`status.${phase}` as never)}</h4>
                    ...
                    {items.map((item) => (
                      <div key={item.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{t('assignedTo')}: {item.assignedTo} | {t('dueDate')}: {item.dueDate}</p>
                        </div>
