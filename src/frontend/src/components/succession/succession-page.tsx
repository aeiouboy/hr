                ...
                <div className="lg:w-1/3">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500">{plan.yearsInRole} {t('yearsInRole')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 mt-2">
                    <Badge variant={RISK_VARIANT[plan.riskLevel]}>{t(`flightRisk.${plan.riskLevel}` as never)}</Badge>
                    <span className="text-xs text-gray-500 dark:text-gray-400">{plan.riskReason}</span>
                  </div>
                </div>

                {/* Successors */}
                <div className="flex-1">
                  <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">{t('identifiedSuccessors')} ({plan.successors.length})</h4>
                  {plan.successors.length === 0 ? (
                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('noSuccessorsIdentified')}</p>
                  ) : (
                    <div className="space-y-3">
                      {plan.successors.map((s) => (
                        <div key={s.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                          {s.photo && <img src={s.photo} alt="" className="w-8 h-8 rounded-full" />}
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">{s.name}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">{s.position} - {s.department}</p>
                          </div>
