...
Subject: A stylized person silhouette (head circle + torso) drawn using circuit board trace lines — thin right-angle paths with small node dots at corners, like a PCB schematic. The person shape is formed entirely from these tech traces. Suggests HR-tech, data-driven people management, digital workforce.

Style: Flat vector illustration, tech-modern but warm (not sterile blue-chrome). Enterprise SaaS with clear tech DNA. Logo-mark quality.

Color palette: circuit trace lines in warm amber (#F4A261) with node dots in warm teal (#2A9D8F). The silhouette outline that emerges in warm coral (#FF8A80) but NOT red. Optional inner fill in very light peach (#FFE5D9). Absolutely NO red, NO crimson.

Composition: person silhouette centered, 15% whitespace margin, no text, no tagline, pure white background, readable at 64x64 favicon size.
